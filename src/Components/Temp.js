...
const Temp = (props) => {
    ...
    const Process = (e) => {
        ...
        if (props.user) {
            ...
            fData.append("filename", e.target.file.files[0].name);
            ...
            fetch("http://localhost:3000/compress", { method: "POST", body: fData })
                ...
                .then(res => {
                    ...
                    const uploadTask = uploadBytesResumable(storageRef, new File([res], uname + "$" + e.target.file.files[0].name), metadata);
                    ...
                    const link = document.createElement('a');
                    link.href = data;
                    link.download = "Compressed_" + e.target.file.files[0].name;
            ...
            fData2.append("file", e.target.file.files[0]);
            fData2.append("filename", e.target.file.files[0].name);
    
            
            fetch("http://localhost:3000/compress/stat", { method: "POST", body: fData2 })
                .then(resp => resp.json())
                .then(resp => {
                    console.log(resp);
                    ...
                    uploadTask.on('state_changed',
                        ...
                        () => {
                            ...
                            getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                                ...
                                if (resp) {
                                    set(dbref(db, "userfiles/" + e.target.file.files[0].name.split(".")[0]), {
                                        filename: e.target.file.files[0].name,
                                        now: resp.size_output,
                                        prev: resp.size_in,
                                        url: downloadURL
                                    })
                                        .then(() => console.log("Upload Sucess !"))
...
}, [userfileDetails,trigger]);




useEffect(() => { }, [setisProcess, setLoad , settrigger]);
                                    ...
                                    <Table hover responsive style={{ color: "white" }}>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <center> {stat ? ((((stat.size_output / 1024) / 1024).toFixed(2)/((stat.size_in / 1024) / 1024).toFixed(2))*100).toFixed(2) + " %" : "-"}</center>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </Table>
